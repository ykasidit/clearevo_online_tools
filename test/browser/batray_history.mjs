...
await evalJs(`(() => {
  ...
  for (let i = 120; i >= 1; i--) rows.push({ t: now - i * 1000, p: 'n11', soc: 50 + (i % 10), v: 52.1, i: i % 2 ? 12 : -18, w: i % 2 ? 620 : -930, ah: 150, tm: 30, t1: 25, t2: 26, ch: 1, ds: 1, bal: 0, err: 0, c: null });   // the last two minutes: always today's UTC day
...
const box = await evalJs(`({ use: document.getElementById('stUse').textContent, hist: document.getElementById('stHistSize').textContent, set: document.getElementById('stSetSize').textContent, logs: document.getElementById('stLogSize').textContent, note: document.getElementById('histNote').textContent, btns: ['histBrowse', 'histBackup', 'histRestore', 'histClear', 'setBrowse', 'setBackup', 'setRestore', 'setReset', 'logBrowse', 'logDownload', 'logClear'].map((id) => [id, !document.getElementById(id).hidden]) })`);
check('the Storage box: used of the maximum with a percent on top, one row each for history data, settings and debug logs with their sizes and buttons, the long text shortened below', /^[\d.]+ (KB|MB) of [\d.]+ (GB|MB) \([\d.]+ %\)$/.test(box.use) && /^\d+ KB · \d+ days? since \d{4}-\d{2}-\d{2}$/.test(box.hist) && /^\d+ KB · \d+ values?$/.test(box.set) && /(KB|MB) · \d+ files? · this session/.test(box.logs) && /oldest days go when less than 100 MB stay free/.test(box.note) && box.btns.every(([, shown]) => shown), box);
const setBk = await evalJs(`(async () => { const cou = URL.createObjectURL.bind(URL); URL.createObjectURL = (b) => { window.__setBlob = b; return cou(b); }; HTMLAnchorElement.prototype.click = function () { window.__setName = this.download; window.__downloadName = this.download; }; const r = window.__batrayTest.backupSettings(); const j = JSON.parse(await window.__setBlob.text()); return { r, name: window.__setName, app: j.app, keys: Object.keys(j.settings) }; })()`);
check('Settings back up as batray-settings-<day>.json holding only the app keys', setBk.r && /^batray-settings-\d{4}-\d{2}-\d{2}\.json$/.test(setBk.name) && setBk.app === 'BatRay' && setBk.keys.length === setBk.r.count && setBk.keys.every((k) => /^(batray_|ce_zoom)/.test(k)), setBk);
const setRs = await evalJs(`(async () => { localStorage.setItem('batray_cutoff_pct', '33'); const r = await window.__batrayTest.restoreSettings(JSON.stringify({ app: 'BatRay', settings: { batray_cutoff_pct: '17', evil: 'x' } }), 'test'); return { r, cutoff: localStorage.getItem('batray_cutoff_pct'), evil: localStorage.getItem('evil'), bad: await window.__batrayTest.restoreSettings('{"app":"Other"}', 'test') }; })()`);
check('Restore applies only the app keys from a checked file, and refuses another file', setRs.r && setRs.r.restored === 1 && setRs.r.skipped === 1 && setRs.cutoff === '17' && setRs.evil === null && setRs.bad === null, setRs);
...
const hb = await evalJs(`(async () => { document.getElementById('histBrowse').click(); await new Promise((r) => setTimeout(r, 400)); const items = [...document.querySelectorAll('#sheetItems .item')].map((i) => [i.querySelector('.nm').textContent, i.querySelector('.sz').textContent]); const yday = new Date(Date.now() - 86400e3).toISOString().slice(0, 10); await window.__batrayTest.browseDelete(yday); await new Promise((r) => setTimeout(r, 300)); const after = [...document.querySelectorAll('#sheetItems .item .nm')].map((n) => n.textContent); const list = await window.__batrayTest.histList(); document.querySelector('#sheetActs [data-act=close]').click(); return { items, after, list: list.map((d) => d.day), yday }; })()`);
check('Browse lists the day files (today marked live) and deletes one day on its own', hb.items.length === 2 && hb.items.some(([n]) => /today, live/.test(n)) && hb.items.some(([n]) => n === hb.yday + '.ndjson.gz') && hb.after.length === 1 && !hb.list.includes(hb.yday), hb);
