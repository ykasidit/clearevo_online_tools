...
test('a refused lock (battery saver) or a browser without the API starts the video at once in auto; the delay caps at 30 s', () => {
  const ws = wakeState(true); ws.wanted = true;
  wakeRefused(ws); assert.equal(ws.refusals, 1); assert.equal(wakeVideoWanted(ws), true);
  for (let i = 0; i < 10; i++) wakeRefused(ws);
  assert.equal(wakeRetryDelayMs(ws), WAKE_RETRY_MAX_MS);
  const none = wakeState(false); none.wanted = true;
  assert.equal(wakeShouldRequest(none, true), false); assert.equal(wakeVideoWanted(none), true);
});

test('never and always override the counters; an unknown saved mode falls back to auto', () => {
  const ws = wakeState(true); ws.wanted = true;
  ws.mode = 'always'; assert.equal(wakeVideoWanted(ws), true);
  ws.mode = 'never'; wakeRefused(ws); ws.drops = 5; assert.equal(wakeVideoWanted(ws), false);
  assert.equal(wakeMode('sometimes'), 'auto'); assert.equal(wakeMode(null), 'auto'); assert.equal(wakeMode('always'), 'always');
